...
class AudioOutput:
    ...
    def play_wake_chime(self) -> None:
        """Play Siri-style rising dual chime when assistant activates."""
        if not self._available:
            return
        try:
            import sounddevice as sd
            ...
            tone1 = np.sin(2 * np.pi * 587.33 * t1) * env1 * 0.14
            tone2 = np.sin(2 * np.pi * 880.00 * t2) * env2 * 0.16
            chime = np.concatenate([tone1, tone2]).astype(np.float32)
            sd.play(chime, samplerate=sr, device=self._device, blocking=False)
        except Exception as e:
            log.debug("audio.wake_chime_error", error=str(e))

    def play_done_chime(self) -> None:
        """Play soft confirmation chime upon task completion."""
        if not self._available:
            return
        try:
            import sounddevice as sd
            ...
            tone = (np.sin(2 * np.pi * 783.99 * t) * env * 0.12).astype(np.float32)
            sd.play(tone, samplerate=sr, device=self._device, blocking=False)
        except Exception as e:
            log.debug("audio.done_chime_error", error=str(e))
