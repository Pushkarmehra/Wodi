...
// ── Woody SSE/IPC bridge (browser shim) ────────────────────────────────────────
if (!window.Woody) {
      ...
      try {
        ...
        es.onerror = () => {
          updateListeners.forEach(cb => cb({
            type: 'error',
            agent: 'System',
            message: 'Connection to Woody lost. Is the server running?',
          }));
          es.close();
        };
        return { success: true };
...
async function run() {
  ...
  try {
    const res = await window.Woody.executePrompt(prompt);
...
window.Woody.onAgentUpdate(d => {
  switch (d.type) {
    ...
    case 'response':
      addMessage(d.message, 'agent', d.agent || 'Woody');
      setDot('idle');
      break;
    ...
    case 'success':
      if (d.message) addMessage(d.message, 'agent', d.agent || 'Woody');
      setDot('idle');
      break;
...
btnClose.addEventListener('click', () => {
  resetPanel();
  window.Woody.clearHistory();
  window.Woody.hideWindow();
});

// Voice button — Web Audio API Live Microphone Toggle & STT
let rendererMicStream = null;
let rendererAudioCtx = null;

btnVoice.addEventListener('click', async () => {
  if (btnVoice.classList.contains('recording')) {
    if (rendererMicStream) {
      rendererMicStream.getTracks().forEach(t => t.stop());
      rendererMicStream = null;
    }
    if (rendererAudioCtx && rendererAudioCtx.state !== 'closed') {
      rendererAudioCtx.close().catch(() => {});
      rendererAudioCtx = null;
    }
    btnVoice.classList.remove('recording');
    btnVoice.title = 'Voice Input (M)';
    setDot('idle');
  } else {
    try {
      rendererMicStream = await navigator.mediaDevices.getUserMedia({ audio: true });
      rendererAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
      if (rendererAudioCtx.state === 'suspended') await rendererAudioCtx.resume();
      btnVoice.classList.add('recording');
      btnVoice.title = 'Mute Voice Input';
      setDot('active');
      expandPanel();
      addMessage('🎙️ Microphone active. Speak naturally or type your request below.', 'agent', 'Woody');
    } catch (err) {
      btnVoice.classList.add('recording');
      setDot('active');
      expandPanel();
      addMessage('🎙️ Voice mode active. Listening for "Hey Woody" or press Ctrl+Space.', 'agent', 'Woody');
      setTimeout(() => {
        btnVoice.classList.remove('recording');
        setDot('idle');
      }, 4000);
...
window.Woody.onWindowFocused(() => input.focus());
