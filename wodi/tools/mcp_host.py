...
from wodi.tools.mcp_manifest import ManifestLoader, PermissionDeniedError, PermissionTier, PluginManifest
...
class MCPHost:
    """
    Central MCP host that manages all tool plugins for Wodi.

    Usage:
        host = MCPHost(plugin_dir="plugins", confirm_callback=my_confirm_fn)
        host.start()
        result = await host.invoke("wodi-system-tools", "get_time_date", {})
    """
    ...
    async def _dispatch(self, plugin_name: str, tool_name: str, arguments: dict) -> Any:
        ...
        from wodi.tools.builtin import desktop_tools, system_tools
        builtin_map = {
