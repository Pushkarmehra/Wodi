"""
Wodi WebEngine Overlay — PySide6 QWebEngineView window serving the OLED HTML UI.

Ported from the Nex prototype's NexMainWindow (gui/app.py) and integrated into
the Wodi application layer. Uses the FastAPI SSE backend via localhost.
...
Features:
  ...
  - Ctrl+Space      — toggle window (legacy Wodi hotkey)
...
log = logging.getLogger("WodiWebOverlay")
...
class WodiWebOverlay(QMainWindow):
    """
    Frameless WebEngine window hosting the Wodi OLED HTML interface.

    Communicates with the renderer via:
      - URL: serves content from the Wodi FastAPI backend
    ...
    def _on_title_changed(self, title: str) -> None:
        ...
        if "action:hide" in title:
            self.hide_window()
            self.web_view.page().runJavaScript("document.title = 'Wodi';")
...
def _make_tray_icon() -> QIcon:
    """Generate a Wodi indigo circle tray icon."""
...
def start_web_overlay(server_url: str = "http://127.0.0.1:8765/") -> None:
    """
    Launch the Wodi WebEngine overlay as a standalone PySide6 application.
    ...
    window = WodiWebOverlay(server_url=server_url)
    ...
    menu = QMenu()
    menu.addAction("Toggle Wodi (Ctrl+Alt+Space)").triggered.connect(window.toggle_window)
    menu.addAction("Focus Input (Ctrl+/)").triggered.connect(window.focus_input)
    menu.addSeparator()
    menu.addAction("Quit Wodi").triggered.connect(lambda: os._exit(0))
    tray.setContextMenu(menu)
    tray.activated.connect(
        lambda r: window.toggle_window()
        if r == QSystemTrayIcon.ActivationReason.Trigger else None
    )
    tray.setToolTip("Wodi AI — Ctrl+Alt+Space")
    tray.show()
